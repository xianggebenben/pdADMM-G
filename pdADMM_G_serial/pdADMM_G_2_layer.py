...
for seed_num in seed_nums:
    ...
    if os.path.exists('pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed_num) + '_2layers.pt')\
            and os.path.exists('pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed_num) + '_2layers_acc.pt'):
        f = torch.load('pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed_num) + '_2layers.pt')
        print('loaded: {}'.format('pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed_num) + '_2layers.pt'))
        min_epoch = f['i']+1
        W1 = f['W1']
        b1 = f['b1']
        z1 = f['z1']
        ...
        rho = f['rho']
        del f
        g = torch.load('pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed_num) + '_2layers_acc.pt')
        train_acc = g['train_acc']
        train_cost = g['train_cost']
        val_acc = g['val_acc']
        val_cost = g['val_cost']
        linear_r = g['linear_r']
        obj = g['obj']
    ...
    for i in range(min_epoch, ITER):
        ...
        torch.save(
            {'i':i,'rho':rho,
             "W1": W1.cpu(), "b1": b1.cpu(), "z1": z1.cpu(), "q1": q1.cpu(), "u1": u1.cpu(),
             'p2':p2.cpu(), "W2": W2.cpu(), "b2": b2.cpu(), "z2": z2.cpu()}, \
            './pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed_num) + '_2layers.pt')
        torch.save(
            {"linear_r": linear_r.cpu(), "obj": obj.cpu(),
             "train_acc": train_acc, "train_cost": train_cost, "val_acc": val_acc, "val_cost": val_cost
                , 't_train': t_train},
            './pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed_num) + '_2layers_acc.pt')
    (test_acc, test_cost) = test_accuracy(W1.cpu(), b1.cpu(), W2.cpu(), b2.cpu(), x_test.cpu(), y_test.cpu())
