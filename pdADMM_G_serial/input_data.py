from torch_geometric.datasets import Planetoid, Amazon, Coauthor, Flickr, Reddit2
import numpy as np
import torch
...
class coauthor_physics():
    def __init__(self):
        self.data = Coauthor(root='./dataset/physics', name='physics')[0]
        self.dataset = Coauthor(root='./dataset/physics', name='physics')
        self.label = self.data.y
        self.data.y = onehot(self.data.y)
        self.num_classes = self.data.y.size()[1]
        self.adj = gcn_norm(self.data.edge_index)
        split_dataset(self)
        self.y_train, self.y_test, self.y_val = self.data.y[self.train_mask], self.data.y[self.test_mask], self.data.y[self.val_mask]
        data_augmentation(self,'coauthor_physics')
        self.x_train, self.x_test, self.x_val = self.x[self.train_mask], self.x[self.test_mask], self.x[
            self.val_mask]
...
def data_augmentation(data, name=None):
    if name and os.path.exists('/home/cc/'+name+'_preprocess.pt'):
        f = torch.load('/home/cc/'+name+'_preprocess.pt')
        data.x = f['x']
