...
def Net(label, num_of_neurons, seed):
    data = torch.load("pdadmm_g_"+dataset_name+"_" + str(num_of_neurons)+"_"+repr(seed) + "_5layers.pt")
...
for seed in seed_nums:
    ...
    min_epoch = 0
    if os.path.exists('pdadmm_g_'+dataset_name+'_' + repr(num_of_neurons) + '_'+repr(seed) +'_10layers.pt')\
            and os.path.exists('pdadmm_g_'+dataset_name+'_' + repr(num_of_neurons)+'_'+repr(seed) + '_10layers_acc.pt'):
        f = torch.load('pdadmm_g_'+dataset_name+'_' + repr(num_of_neurons) + '_'+repr(seed) +'_10layers.pt')
        min_epoch = f['i'] + 1
        W1 = f['W1'].to(device)
        b1 = f['b1'].to(device)
        z1 = f['z1'].to(device)
        ...
        p = torch.load('pdadmm_g_'+dataset_name+'_' + repr(num_of_neurons)+'_'+repr(seed) + '_10layers_acc.pt')
        linear_r = p["linear_r"]
        obj = p["obj"]
    ...
    for i in range(min_epoch, ITER):
        ...
        torch.save(
            ...
            './pdadmm_g_'+dataset_name+'_' + repr(num_of_neurons) + '_'+repr(seed) +'_10layers.pt')

        torch.save(
            {"linear_r": linear_r, "obj": obj,
             "train_acc": train_acc, "train_cost": train_cost, "val_acc": val_acc, "val_cost": val_cost,
              't_train': t_train},
            './pdadmm_g_' + dataset_name + '_' + repr(num_of_neurons) + '_' + repr(seed) + '_10layers_acc.pt')
