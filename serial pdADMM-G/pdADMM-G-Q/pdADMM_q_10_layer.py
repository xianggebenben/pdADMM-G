...
from serial.input_data import cora, pubmed, citeseer, amazon_photo, amazon_computers, coauthor_cs
import torch.nn.functional as F


# initialize the neural network
def Net(label, num_of_neurons):
    data = torch.load("pdadmm_amazon_photo_" + str(num_of_neurons) + "_5layers.pt")
    W1 = data['W1']
    b1 = data['b1']
    z1 = data['z1']
    q1 = data['q1']
...
obj = np.zeros(ITER)
#cora rho=1e-4 mu=0.01
#pubmed rho=1e-4 mu=1
#citeseer rho=1e-4 mu=0.01
#amazon computers rho=1e-4 mu=0.01
...
#     'pdadmm_q_amazon_photo_' + repr(num_of_neurons) + '_10layers.pt')

# for plot use
# torch.save(
#     {  "linear_r": linear_r, "obj": obj},
#     'pdadmm_q_amazon_photo_' + repr(num_of_neurons) + '_10layers_obj.pt')

torch.save(
    {  "train_acc": train_acc, "train_cost": train_cost, "test_acc": test_acc, "test_cost": test_cost},
    'pdadmm_q_amazon_photo_' + repr(num_of_neurons) + '_10layers_acc.pt')